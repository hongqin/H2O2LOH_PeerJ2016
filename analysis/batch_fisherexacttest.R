# Aug 29, 2016. Changed to one-tailed Fisher exact test. 
...
rm=(list=ls())
setwd("~/github/H2O2LOH_PeerJ2016/analysis")
debug = 0;
...
for( ii in 1:length(FileList)) {  
  ...
  for( i in 1:length(tbMerge[,1])) {
   if ( (tbMerge$halfBlack[i] >=3) & (tbMerge$quarterBlack[i]>=3) & (tbMerge$ThreeQBlack[i]>=3)) {
    ...
    tryCatch(
      {
        tmp = fisher.test(mytab, alternative='greater'); #one-tailed fisher exact test
...
dim( output[output$nonsignificant_fishertest==0, ] )
dim( output[output$nonsignificant_fishertest==1, ] )
dim( output[output$nonsignificant_fishertest>1, ] )

output[output$nonsignificant_fishertest>1, ] 
